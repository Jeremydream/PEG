...
if __name__ == '__main__':
    ff = 'forcefield.xml'
    pdb_AB = PDBFile('coc_dimer.pdb')
    pdb_A = PDBFile('coc.pdb')
    pdb_B = PDBFile('coc.pdb')
    H_AB = Hamiltonian(ff)
    H_A = Hamiltonian(ff)
    H_B = Hamiltonian(ff)
    pme_generator_AB, \
            ...
            dhf_generator_AB = H_AB.getGenerators()
    pme_generator_A, \
            disp_generator_A, \
            ex_generator_A, \
            sr_es_generator_A, \
            sr_pol_generator_A, \
            sr_disp_generator_A, \
            dhf_generator_A = H_A.getGenerators()
    pme_generator_B, \
            disp_generator_B, \
            ex_generator_B, \
            sr_es_generator_B, \
            sr_pol_generator_B, \
            sr_disp_generator_B, \
            dhf_generator_B = H_B.getGenerators()
    ...
    def MSELoss(params, scan_res):
        ...
        weights_pts = jnp.piecewise(E_tot_full, [E_tot_full<10, E_tot_full>=10], [lambda x: jnp.array(1.0), lambda x: jnp.exp(-(x-10)/kT)])
        ...
        for ipt in range(npts):
            ...
            energies['es'] = energies['es'].at[ipt].set(E_sr_es)
            energies['pol'] = energies['pol'].at[ipt].set(E_sr_pol)
            energies['disp'] = energies['disp'].at[ipt].set(E_sr_disp)
            energies['dhf'] = energies['dhf'].at[ipt].set(E_dhf)
            energies['tot'] = energies['tot'].at[ipt].set(E_ex + E_sr_es + E_sr_pol + E_sr_disp + E_dhf)
    ...
    with open('params.0.pickle', 'rb') as ifile:
